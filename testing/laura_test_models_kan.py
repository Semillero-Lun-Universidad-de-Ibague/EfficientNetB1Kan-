...
if __name__ == '__main__':
    ...
    params = {
        'grid_size': params_nni.get('grid_size', 64),
        'spline_order': params_nni.get('spline_order', 3),
        'scale_noise': params_nni.get('scale_noise', 0.97),
        'scale_base': params_nni.get('scale_base', 0.97),
        'scale_spline': params_nni.get('scale_spline', 0.97)
    }
    ...
    if args.model_name == 'vgg':
        ...
        test_model(model, 'VGG16_KAN_10epochs_{}'.format(date.today()), num_epochs=1, progress_bar=False)
    elif args.model_name == 'resnext':
        resnext = resnext50_32x4d(pretrained=True)
        test_model(resnext, 'ResNext_10epochs_{}'.format(date.today()), num_epochs=1, progress_bar=False)
    elif args.model_name == 'resnext_kan':
        model = ResNext_KAN(4, params)
        test_model(model, 'ResNext_KAN_10epochs_{}'.format(date.today()), num_epochs=1, progress_bar=False)
    elif args.model_name == 'efficientnet':     
        model = efficientnet_b1(pretrained=True)
        test_model(model, 'EfficientNet_10epochs_{}'.format(date.today()), num_epochs=1, progress_bar=False)
